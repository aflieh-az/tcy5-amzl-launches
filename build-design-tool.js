...
$('xup').addEventListener('change',function(ev){
  ...
  const daysInput=prompt('How many days does this data cover?\\n(Volume will be divided by this number to get daily average)','21');
    ...
    try{
      ...
      if(palletSheet){
        ...
        for(const row of rows){
          ...
          const adv=Math.round((parseFloat(String(rawAdv).replace(/,/g,''))||0)/days);
      ...
      if(ruleSheet){
        ...
        if(palletSheet){
          ...
          for(const pr of pRows){
            ...
            const pAdv=Math.round((parseFloat(String(rawAdv).replace(/,/g,''))||0)/days);
      ...
      render();upIns();
      lg('\\ud83d\\udcc2 Uploaded layout: '+updated+' chutes updated from '+file.name+' ('+days+' day avg)');
      alert('Layout updated: '+updated+' chutes refreshed from '+file.name+'\\nVolume divided by '+days+' days for daily average.');
    }catch(err){
...
$('bss').addEventListener('click', ()=>{
  ...
  try{
    ...
    const CW=62,CH=42,PAD=16,HDR=36,LGH=40,RH=20,RW=50;
    const cols=LN.length,rows=src.length;
    const totalW=RW+cols*CW+PAD*2;
    const totalH=PAD+HDR+LGH+RH+rows*CH+PAD;
    const cv=document.createElement('canvas');cv.width=totalW*2;cv.height=totalH*2;
    const ctx=cv.getContext('2d');ctx.scale(2,2);
    // Background
    ctx.fillStyle='#0b0b1e';ctx.fillRect(0,0,totalW,totalH);
    // Title
    ctx.fillStyle='#e8e8f0';ctx.font='bold 14px Inter,sans-serif';
    ctx.fillText('TCY5 Floor Layout — '+new Date().toLocaleDateString(),PAD,PAD+16);
    // Legend
    const lgY=PAD+HDR;
    const cats=Object.entries(WC).filter(([k])=>{for(const row of src)for(const cell of row.cells)if(cell.cat===k)return true;return false});
    let lx=PAD;
    ctx.font='bold 9px Inter,sans-serif';
    for(const[k,v]of cats){
      ctx.fillStyle=v.b;ctx.fillRect(lx,lgY,12,12);
      ctx.strokeStyle='rgba(255,255,255,0.2)';ctx.strokeRect(lx,lgY,12,12);
      ctx.fillStyle='#c0c0d0';ctx.fillText(v.l,lx+16,lgY+10);
      lx+=ctx.measureText(v.l).width+30;
      if(lx>totalW-60){lx=PAD;}
    }
    // Column headers (Lane numbers)
    const gridTop=lgY+LGH;
    ctx.font='bold 8px Inter,sans-serif';ctx.fillStyle='#8888aa';ctx.textAlign='center';
    for(let c=0;c<cols;c++){
      const x=PAD+RW+c*CW+CW/2;
      ctx.fillText('L'+LN[c],x,gridTop+14);
    }
    // Rows
    const cellTop=gridTop+RH;
    for(let r=0;r<rows;r++){
      const y=cellTop+r*CH;
      // Row label
      ctx.textAlign='center';ctx.fillStyle='#8888aa';ctx.font='bold 8px Inter,sans-serif';
      ctx.fillText('R'+src[r].rn,PAD+RW/2,y+CH/2+3);
      for(let c=0;c<src[r].cells.length;c++){
        const cell=src[r].cells[c];
        const x=PAD+RW+c*CW;
        const co=WC[cell.cat]||WC.EMPTY;
        // Cell background
        ctx.fillStyle=co.b;ctx.fillRect(x+1,y+1,CW-2,CH-2);
        // Cell border
        ctx.strokeStyle='rgba(42,42,85,0.6)';ctx.strokeRect(x+1,y+1,CW-2,CH-2);
        // Changed indicator
        if(!showOrig&&isChanged(r,c)){ctx.strokeStyle='#ff9900';ctx.lineWidth=2;ctx.strokeRect(x+1,y+1,CW-2,CH-2);ctx.lineWidth=1}
        // Type badge (F/M)
        ctx.font='bold 7px Inter,sans-serif';ctx.textAlign='right';
        ctx.fillStyle=cell.fl?'#ff6b6b':'#4ecdc4';
        ctx.fillText(cell.fl?'F':'M',x+CW-4,y+10);
        // Chute ID
        ctx.textAlign='center';ctx.fillStyle=co.t;ctx.font='bold 7px Inter,sans-serif';
        ctx.fillText(String(cell.id),x+CW/2,y+14);
        // Filter name (truncated)
        ctx.font='6px Inter,sans-serif';
        const fn=cell.f||'';const tfn=fn.length>10?fn.slice(0,9)+'…':fn;
        ctx.fillText(tfn,x+CW/2,y+24);
        // ADV
        if(cell.adv!=null){ctx.font='6px Inter,sans-serif';ctx.globalAlpha=0.7;ctx.fillText(String(cell.adv),x+CW/2,y+33);ctx.globalAlpha=1}
      }
    }
    // Download
